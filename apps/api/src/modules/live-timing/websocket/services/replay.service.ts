...
export class ReplayService {
  ...
  private readonly CHUNK_DURATION = 300; // 5분 (초)
  private readonly MAX_CACHE_SIZE = 50; // 최대 50개 청크 (약 4시간분)
  private readonly PRELOAD_CHUNKS = 2; // 현재 청크 + 다음 2개 청크 미리 로드
  ...
  async startReplay(
    ...
    if (existingSession) {
      ...
      if (existingSession.replayState === 'PAUSED') {
        ...
        existingSession.startTime =
          Date.now() - (existingSession.currentTimestamp * 1000) / speed;
    ...
    try {
      ...
      if (timeRange[0]?.min_time !== null && timeRange[0]?.max_time !== null) {
        ...
        this.logger.log(`✅ 세션 ${sessionId} 실제 시간 범위: ${minSessionTime}초 ~ ${maxSessionTime}초`);
      } else {
        this.logger.warn(`⚠️ 세션 ${sessionId} 시간 범위 데이터 없음, 기본값 사용`);
      }
    } catch (error: unknown) {
      this.logger.warn(`세션 ${sessionId} 시간 범위 조회 실패, 기본값 사용:`, error instanceof Error ? error.message : String(error));
    }

    const totalDuration = maxSessionTime - minSessionTime;

    this.logger.log(`세션 ${sessionId} 시간 범위: ${minSessionTime}초 ~ ${maxSessionTime}초, 총 길이: ${totalDuration}초`);
    ...
    this.logger.log(`세션 ${sessionId} 초기화 완료. 시작 시간: ${actualStartTimestamp}초 (실제 DB 시간: ${minSessionTime}초부터)`);
  ...
  private startReplayTimer(sessionId: number): void {
    ...
    // 첫 번째 브로드캐스트 즉시 실행
    this.broadcastTimingUpdate(sessionId).catch((error) => {
      this.logger.error(`초기 브로드캐스트 실패 (세션 ${sessionId}):`, error);
    });
    ...
    sessionData.intervalId = setInterval(async () => {
      ...
      if (maxSessionTime && minSessionTime) {
        ...
        if (sessionData.currentTimestamp >= maxNormalizedTime) {
          this.logger.log(`세션 ${sessionId} 종료 (${sessionData.currentTimestamp.toFixed(1)}s >= ${maxNormalizedTime.toFixed(1)}s). 리플레이 중지.`);
      ...
      // 브로드캐스트 실행 로깅 추가
      this.logger.debug(`세션 ${sessionId} 타이밍 업데이트 전송: ${sessionData.currentTimestamp.toFixed(1)}초`);
      await this.broadcastTimingUpdate(sessionId);
    }, 1000); // 500ms에서 1000ms로 증가 (DB 부하 더 감소)

    this.logger.log(`세션 ${sessionId} 재생 타이머 시작됨`);
  }
  ...
  async seekReplay(sessionId: number, seekTimestamp: number): Promise<void> {
    ...
    sessionData.startTime =
      Date.now() - (validNormalizedTimestamp * 1000) / sessionData.speed;
  ...
  changeReplaySpeed(sessionId: number, speed: number): void {
    ...
    sessionData.startTime =
      Date.now() - (sessionData.currentTimestamp * 1000) / validSpeed;
  ...
  unsubscribeClient(sessionId: number, clientId: string): void {
    ...
    if (sessionData.subscribedClients.size === 0) {
      this.logger.log(
        `세션 ${sessionId}의 모든 클라이언트 연결 해제됨. 재생 정지.`,
      );
      this.stopReplay(sessionId);
    }
  }
   ...
   */
  private normalizedTimeToDbTime(normalizedTime: number, sessionInfo: { minSessionTime: number }): number {
  ...
  private async preloadChunk(sessionId: number, chunkIndex: number): Promise<void> {
    ...
    try {
      ...
      this.logger.debug(`청크 ${chunkIndex} 미리 로딩 시작 (정규화: ${normalizedStartTime}s-${normalizedEndTime}s, DB: ${dbStartTime}s-${dbEndTime}s)`);
      
      // 청크 시간 범위의 모든 타이밍 데이터 로드
      const chunkData: DriverTimingData[] = [];
      const sampleInterval = 60; // 60초 간격으로 샘플링
      
      for (let normalizedTime = normalizedStartTime; normalizedTime < normalizedEndTime; normalizedTime += sampleInterval) {
        try {
          ...
          if (timingData.length > 0) {
            ...
            timingData.forEach(data => {
              (data as DriverTimingData & { normalizedTimestamp: number }).normalizedTimestamp = normalizedTime;
            });
            chunkData.push(...timingData);
          }
        } catch (error) {
          this.logger.debug(`정규화 시간 ${normalizedTime}초 데이터 로드 실패:`, error instanceof Error ? error.message : 'Unknown error');
      ...
      this.logger.debug(`청크 ${chunkIndex} 미리 로딩 실패:`, error instanceof Error ? error.message : 'Unknown error');
    } finally {
  ...
  private async broadcastTimingUpdate(sessionId: number): Promise<void> {
    ...
    try {
      ...
      if (shouldFetchRealData) {
        try {
          ...
          if (newDriverData.length > 0) {
            ...
            this.logger.debug(`세션 ${sessionId}: 새 데이터 없음, 기존 데이터 유지 (${driverTimingData.length}개)`);
          }
          ...
          this.logger.warn(`타이밍 데이터 조회 실패 (세션 ${sessionId}):`, error instanceof Error ? error.message : String(error));
      ...
      if (driverTimingData.length > 0 || sessionData.currentTimestamp <= 1.0) { // 첫 1초는 빈 데이터도 허용
        try {
        ...
        this.logger.debug(`세션 ${sessionId}: 빈 데이터로 인한 브로드캐스트 스킵 (시간: ${sessionData.currentTimestamp.toFixed(1)}초)`);
      }
    } catch (error) {
    ...
    try {
      ...
      if (cachedChunk) {
        ...
        cachedChunk.data.forEach(data => {
          const dataNormalizedTime = (data as DriverTimingData & { normalizedTimestamp: number }).normalizedTimestamp;
        ...
        for (const [time] of timeGroups) {
          ...
          if (timeDiff < minTimeDiff && timeDiff <= 30) { // 30초 오차 허용 (10초 → 30초 확대)
        ...
        if (nearestDataList.length > 0) {
          ...
          this.logger.debug(`캐시에서 데이터 반환 (세션 ${sessionId}, 정규화 시간 ${normalizedTimestamp}, 드라이버 ${nearestDataList.length}개)`);
      ...
      this.logger.error(`타이밍 데이터 조회 실패 (세션 ${sessionId}, 정규화 시간 ${normalizedTimestamp}):`, error);
      return [];
    }
  }
  ...
  private mapSimplifiedQueryResultToDriverTiming(simplifiedData: SimplifiedQueryResult[]): DriverTimingData[] {
    return simplifiedData.map((data) => {
        ...
        return {
          ...
          currentLapTime: data.current_lap_time ? Math.round(data.current_lap_time * 1000) : undefined,
  ...
  private async calculateCurrentLap(sessionId: number, normalizedTimestamp: number): Promise<number | undefined> {
    const sessionData = this.activeSessions.get(sessionId);
    ...
    try {
      ...
      if (error && typeof error === 'object' && 'code' in error && error.code === 'P2024') {
        ...
        this.logger.debug(`현재 랩 조회 실패 (정규화 시간: ${normalizedTimestamp}):`, error instanceof Error ? error.message : String(error));
      }
      return 1; // 기본값 반환
    }
  }
  ...
  async getSessionInfo(sessionId: number): Promise<Omit<SessionInfoMessage, 'type' | 'timestamp'> | null> {
